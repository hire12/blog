...
//       toast.success('Article deleted successfully!', { autoClose: 3000 });
//       setArticles((prev) => prev.filter((article) => article.id !== id));
//     } catch (error) {
//       toast.error('Error deleting article. Please try again.', { autoClose: 3000 });
//     }
//   };
...
//       );
//     } catch (error) {
//       toast.error('Error updating article. Please try again.', { autoClose: 3000 });
//     }
//   };
...
//     try {
//       const response = await fetch(`/api/articles?id=${deleteArticleId}`, {
//         method: 'DELETE',
//       });

//       if (!response.ok) {
//         throw new Error(`Failed to delete article with ID: ${deleteArticleId}`);
//       }
...
export default function AdminArticle() {
  const [articles, setArticles] = useState<Article[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null); // Define the error state for UI usage
  const [isEditing, setIsEditing] = useState(false);
  const [currentArticle, setCurrentArticle] = useState<Article | null>(null);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [deleteArticleId, setDeleteArticleId] = useState<string | null>(null);
